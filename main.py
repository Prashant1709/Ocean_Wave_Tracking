...
import datetime
# Get Test Video
cap = cv.VideoCapture('scene1.mp4') #use 0 for realtime video framing
...
while True:
    ...
    for wave in tracked_waves:
        ...
        if wave.update_death(frame_rate, frame_num):
            ...
            data = [[datetime.date.today().year, datetime.date.today().month,datetime.date.today().day, datetime.datetime.now().hour, datetime.datetime.now().minute, datetime.datetime.now().second, wave.moving_speed, 8.14, wave.axis_angle], ]
            with open('data/wave height data/waves_simar_cdf_cal.csv', 'a', encoding='UTF8', newline='') as f:
                writer = csv.writer(f)
                # writer.writerow(header)
                writer.writerows(data)
            cv.imwrite("data/Raw/image_"+str(datetime.datetime.now().year)+str(datetime.datetime.now().month)+str(datetime.datetime.now().day)+str(datetime.datetime.now().hour)+".jpg", frame)
    ...
    frame = cv.circle(frame, (1040, 600), 4, (0, 0, 250), thickness=4)
